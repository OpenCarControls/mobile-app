...
class EgmpDashboardScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<EgmpDashboardScreen> createState() => _EgmpDashboardScreenState();
}
...
class _EgmpDashboardScreenState extends ConsumerState<EgmpDashboardScreen> {
    ...
    return SingleChildScrollView(
      child: Column(
        children: [
          SizedBox(
            height: 350,
            child: _buildVehicleRepresentation(),
          ),
          _buildRightPanel(
            shrinkWrap: true,
  ...
  Widget _buildVehicleRepresentation() {
    ...
    final isChargePortOpen = basicState.chargePortState == BasicState_ChargePortState.CHARGE_PORT_STATE_OPEN;
    final areLightsOn = basicState.areLightsOn;
    final isClimateOn = basicState.isClimateOn;

    final bool isCharging = chargePortState == BasicState_ChargePortState.CHARGE_PORT_STATE_AC_CHARGING ||
                            chargePortState == BasicState_ChargePortState.CHARGE_PORT_STATE_DC_CHARGING;
    final bool isV2L = chargePortState == BasicState_ChargePortState.CHARGE_PORT_STATE_V2L_ACTIVE;
          ...
          Center(
            child: Column(
              ...
              children: [
                ...
                Text(
                  _currentSubMenu == 'climate'
                      ? 'Vehicle Representation\n(Top-down interior view placeholder)'
                      : 'Vehicle Representation\n(2D/3D Asset Placeholder)',
                    ...
                    children: [
                      _buildQuickActionButton(
                        ...
                        onPressed: () {
                          ...
                          ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                        },
                      ),
                      const SizedBox(width: 16),
                      _buildQuickActionButton(
                        icon: isChargePortOpen ? Icons.ev_station : Icons.ev_station_outlined,
                        isActive: isChargePortOpen,
                        onPressed: () {
                          final cmd = BasicCommand(
                            chargePortCommand: ChargePortCommand(open: !isChargePortOpen),
                          );
                          ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                        },
                      ),
                      ...
                      _buildQuickActionButton(
                        ...
                        onPressed: () {
                          final cmd = BasicCommand(
                            climateControlCommand: ClimateControlCommand(climateOn: !isClimateOn),
                          );
                          ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                        },
                      ),
      ...
      child: ListView(
        ...
        children: [
          _buildMenuTile(
            ...
            subtitle: 'Indoor: ${basicState.indoorTemperature.toStringAsFixed(1)}°C',
  ...
  Widget _buildClimateMenu({bool shrinkWrap = false, ScrollPhysics? physics}) {
    ...
    final isClimateOn = basicState.isClimateOn;
    final targetTemp = basicState.targetClimateTemperature == 0 ? 22.0 : basicState.targetClimateTemperature;
      ...
      child: ListView(
        ...
        children: [
          ListTile(
            ...
            title: const Text('Climate & Windows', style: TextStyle(color: Colors.white, fontSize: 18, fontWeight: FontWeight.bold)),
          ),
          Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Text('Outdoor: ${outdoorTemp.toStringAsFixed(1)}°C', style: const TextStyle(color: Colors.white70)),
                Text('Indoor: ${indoorTemp.toStringAsFixed(1)}°C', style: const TextStyle(color: Colors.white70)),
              ],
            ),
          ),
          const SizedBox(height: 32),
          Row(
            ...
            children: [
              _buildClimateIconButton(
                ...
                isActive: isClimateOn,
                onTap: () {
                  final cmd = BasicCommand(climateControlCommand: ClimateControlCommand(climateOn: !isClimateOn));
                  ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                },
              ),
              Row(
                ...
                children: [
                  IconButton(
                    ...
                    onPressed: () {
                      final cmd = BasicCommand(climateControlCommand: ClimateControlCommand(climateOn: isClimateOn, targetTemperature: targetTemp - 0.5));
                      ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                    },
                  ),
                  Text(
                     ...
                     builder: (ctx) => Column(
                       ...
                       children: [
                         ListTile(
                           ...
                           title: const Text('Vent Windows', style: TextStyle(color: Colors.white)),
                           onTap: () {
                             final cmd = BasicCommand(windowCommand: WindowCommand(action: WindowCommand_WindowAction.WINDOW_ACTION_VENT));
                             ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                             Navigator.pop(ctx);
                           },
                         ),
                         ListTile(
                           ...
                           title: const Text('Close Windows', style: TextStyle(color: Colors.white)),
                           onTap: () {
                             final cmd = BasicCommand(windowCommand: WindowCommand(action: WindowCommand_WindowAction.WINDOW_ACTION_CLOSE));
                             ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                             Navigator.pop(ctx);
                           },
                         ),
          ...
          Row(
            ...
            children: [
              _buildLabeledClimateButton(
                ...
                onTap: () {
                  final cmd = BasicCommand(climateControlCommand: ClimateControlCommand(climateOn: isClimateOn, windshieldDefrostOn: !isWindshieldDefrostOn));
                  ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                },
              ),
              _buildLabeledClimateButton(
                ...
                onTap: () {
                  final cmd = BasicCommand(climateControlCommand: ClimateControlCommand(climateOn: isClimateOn, rearWindowDefrostOn: !isRearDefrostOn));
                  ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                },
              ),
              _buildLabeledClimateButton(
                ...
                onTap: () {
                  final cmd = BasicCommand(climateControlCommand: ClimateControlCommand(climateOn: isClimateOn, steeringWheelHeaterOn: !isSteeringHeaterOn));
                  ref.read(vehicleStateProvider.notifier).sendBasicCommand(cmd.writeToBuffer());
                },
              ),
            ],
    ...
    return Container(
      ...
      child: ListView(
        ...
        children: [
          ...
          const ListTile(
            title: Text('Settings coming soon...', style: TextStyle(color: Colors.white54)),
          ),
        ],
      ),
    );
